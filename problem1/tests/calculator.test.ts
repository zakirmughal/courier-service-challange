...
const calculate = (base: number, pkgs: Package[]) => calculator.calculate(base, pkgs);

describe("calculate — sample input", () => {
  const base = 100;

  test("PKG1: OFR001 not applicable (weight 5 < 70)", () => {
    const pkgs: Package[] = [{ id: "PKG1", weight: 5, distance: 5, offerCode: "OFR001" }];
    const [r] = calculate(base, pkgs);
...
describe("calculate — OFR001 boundaries (10%, dist < 200, weight 70–200)", () => {
  const base = 100;
...
describe("calculate — OFR002 boundaries (7%, dist 50–150, weight 100–250)", () => {
  const base = 100;
...
describe("calculate — OFR003 boundaries (5%, dist 50–250, weight 10–150)", () => {
  const base = 100;
...
describe("calculate — general edge cases", () => {
  ...
  test("multiple packages processed independently", () => {
    ...
    const results = calculate(100, pkgs);
